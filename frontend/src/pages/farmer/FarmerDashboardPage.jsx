    ...
    return (<div className="min-h-screen p-8 text-white">
      <div className="container">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-4">
          {['Revenue', 'Orders', 'Inventory', 'AI Insights'].map((item) => (<div key={item} className="card p-5">
              <p className="text-sm text-emerald-200/70">{item}</p>
              <p className="mt-3 text-3xl font-bold">{item === 'Inventory' ? `${products.length}` : item === 'Orders' ? orderCount : item === 'AI Insights' && prediction ? `₹${prediction.predicted_price}` : '—'}</p>
            </div>))}
        </div>
